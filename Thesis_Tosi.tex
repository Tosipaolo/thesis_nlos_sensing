...
Orthogonal frequency division multiplexing (OFDM) radio signals, which are transmitted by base stations to users and vice versa, not only carry data but
...
With the high bandwidths and large antenna apertures available in next-generation mobile networks, facilitate the usage of OFDM radar for gathering sensing information. 
...
The typical assumption is that the target is in line-of-sight (LOS) from the sensing hardware. 
...
The overall objective is to define an initial approach for NLOS radar sensing based on periodogram spectral analysis, which can be exploited for more advanced sensing functionalities.
\\
\\
