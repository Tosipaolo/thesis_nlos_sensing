...
\alert{Positioning of active user equipments (UE) was first deployed with the 3GPP Long Term Evolution (LTE) standard.} LTE introduced support for positioning based on a wide range on methods. 
These methods mostly rely on Cell ID (CID), time of arrival (TOA), Angle of Arrival (AoA) and Global Navigation Satellite System (GNSS). Due to the accuracy requirements in different deployment conditions (\ie indoor or outdoor, urban or rural areas), LTE positioning normally employed two or more methods at the same time \cite{Razavi_Gunnarsson_2018}.
...
For 6G, the radio sensing capabilities should be integrated sharing the same spectrum for both communication and sensing purposes. Figure \ref{fig:isac-scheme-1} illustrates a fundamental sketch of an ISAC scenario as described in \cite{Wild_Grudnitsky_Mandelli_Henninger_Guan_Schaich_2023}. In this depiction a cellular system is shown equipped with array antennas capable of beamforming, serving the dual purpose of sensing and facilitating high-rate, low-latency communication to multiple users.
...
ISAC will benefit from the massive deployment of cellular networks and their coverage: making use of existing hardware and extending its capabilities with sensing, the network will be able to adapt and use information about the surrounding environment as a source of situational knowledge.
...
\section{Technological enablers}
	...
	\begin{itemize}
		...
		\item the new Frequency Range 3 (FR3), not yet specified: likely from 7 to 20 GHz
	\end{itemize}
	
	FR1 is currently used for 5G capacity layers, providing a bandwidth of up to 100 MHz. For 6G, the main capacity layer will be placed in the "Golden Band" in 6-14 GHz, offering up to 400 MHz of bandwidth. FR2 in the millimetre frequency bands will start at 24 GHz and offer significantly more bandwidth, up to 1 GHz.
	
	FR3 and FR2 are the most relevant candidates for defining the sensing spectrum in 6G. Their selection would make sense for wide area sensing and high resolution sensing, respectively.
	
	Feasible use cases and possible scenarios will be driven by the characteristics and specifications offered by the frequency band of choice. It can be expected that FR2 will be used in micro-cellular deployments. They will, \eg, be deployed in indoor environments, such as factory floors. Sensing targets in such environment will likely be pedestrians, passive factory objects and vehicles.
	Macro-cellular environments in the Golden Bands will dominate outdoors, where targets of interest will be vehicles for traffic monitoring, roadside safety and even detection of unmanned aerial vehicles (UAV), such as drones \cite{Mandelli_Henninger_Bauhofer_Wild_2023}.
	...
	Pulsed radar transmits short, typically unmodulated, pulses or bursts of pulses of large bandwidth, followed by a silent period, waiting for the reception of the echoed signal. Continuous wave radars are currently largely used in automotive systems and the main example are frequency modulated continuous wave (FMCW) radars. Such systems transmit chirp signals with fixed or variable frequencies, which get shifted by the so called beat frequency when they get reflected by objects.
	...
	Current mobile communications are dominated by multi-carrier waveforms (OFDM).
	...
	The main disadvantage of OFDM is the high PAPR and the additional \alert{DSP} necessary for performing ISAC data processing.
	
	\subsubsection{Waveforms for next-gen communication networks}
	
	\replace{A major challenge for OFDM and multi-carrier signals is their performance in highly frequency-dispersive channels.}{One of the major challenges for OFDM, and multi-carrier waveforms in general, is the performance loss observed in particular environments characterized by highly frequency-dispersive channels (\eg high mobility scenarios, dynamic wireless networks).}
	...
	Orthogonal Time Frequency Space (OTFS) has been proposed as a modulation effective in tackling these problems. OTFS operates in the delay-Doppler domain. 
	The fading, time-variant channel seen by an OFDM signal is transformed into a non-fading, time-independent channel through a 2D Fourier transform \cite{OTFS_Hadani_2017}. 
	...
	OTFS can be interpreted as the sequence of two transformations. 
	Firstly, the transmitter maps the complex symbols into discrete slots in the delay-Doppler domain.
	Secondly, these symbols are transformed in the time-frequency domain through a 2D inverse symplectic finite Fourier transform (iSFFT). The the time-domain signal is obtained by using the inverse Fourier transform (iFFT). 
	The receiver will perform the inverse operation using FFT and SFFT, respectively.
	OTFS is an suitable waveform for integrating sensing and communication due to its inherent property of operating in the delay-Doppler domain.
...
\section{Proof of concept architecture}
	...
	The system consists in a mmWave 5G radio unit (RU) denoted gNB and a base transciever station (BSU), extended by an additional RU called \textit{Sniffer} and a server called \textit{Sensing Processing Unit} (SPU). 
	The sniffer is synchronized with the gNB by the same synchronization source. 
	The SPU receives the complex IQ signal,  after FFT and removal of the cyclic prefix (CP), as input for further sensing processing on a per-radio frame basis \cite{Wild_Grudnitsky_Mandelli_Henninger_Guan_Schaich_2023}.
