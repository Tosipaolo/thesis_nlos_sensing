...
These methods mostly rely on \gls{cid}, \gls{toa}, \gls{aoa} and \gls{GNSS}. Due to the accuracy requirements in different deployment conditions (\ie indoor or outdoor, urban or rural areas), \gls{lte} positioning normally employed two or more methods at the same time \cite{Razavi_Gunnarsson_2018}.
The fifth generation of cellular networks, with 5G \gls{NR}, expanded this capabilities. The introduction of beam-based operation and massive antenna systems, especially at mmWave, will enable higher positioning accuracy by exploiting more precise angular information in combination with time measurements \cite{Keating_Saily_Hulkkonen_Karjalainen_2019}. 
...
\section{Technological enablers}
	...
	The main disadvantage of OFDM is the high \gls{papr} and the additional \alert{DSP} necessary for performing ISAC data processing.
	
	\subsubsection{Waveforms for next-gen communication networks \alert{TODO}}
	...
	In 5G, beamforming commonly employs an all-digital approach at lower frequencies. However, for mmWave frequencies and above, analogue or hybrid beamforming is typically used. 
...
\alert{	Recent years have seen \gls{ai} and \gls{ml} emerging as a novel and powerful method for extracting and processing information effectively, often surpassing traditional methods.}
	In the radar world \gls{ai} has been used to solve target classification problems. 
	\Gls{ai} systems can accurately differentiate between various targets, such as aircrafts, ships, and ground vehicles, based on their radar signatures. 
...
\section{Proof of concept architecture}
	...
	The \gls{spu} receives the complex IQ signal,  after FFT and removal of the \gls{cp}, as input for further sensing processing on a per-radio frame basis \cite{Wild_Grudnitsky_Mandelli_Henninger_Guan_Schaich_2023}.
	...
	Option 7-2 splits the network at the physical layer (PHY) level, which handles the conversion of digital data to the analogue radio signal in \gls{dl}, while doing the opposite in \gls{ul}.
	...
	The system is equipped with analog beamforming and measurements can be obtained using a fixed beam for both \gls{gnb} \gls{ru} and sniffer.
...
\section{Sensing use-cases}
	...
	Feasibility of monostatic \gls{isac} in \gls{nlos} environments is still a rather unexplored area of research \textit{\alert{missing ref}}, as commonly studied use cases assume the target to be in LOS.
