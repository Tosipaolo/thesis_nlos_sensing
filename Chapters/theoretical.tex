...
\section{OFDM radar processing}
    ...
    The received frame matrix $\mathbf F_{Rx}$ is obtained by analog/digital conversion of the signal, removal of \gls{cp} from each OFDM symbol, and computing the FFT of length $N_{\text{subcarriers}}$ for every column of the matrix.
...
\section{Periodogram-based estimation algorithms}
    ...
    The input data for the periodogram is the \gls{csi} $\bm{F}$, and the periodogram is estimated in two dimensions
    ...
    As demonstrated by Braun, computing the periodogram results in a \gls{snr} gain of a factor $NM$. The numerator in Equation~\eqref{eq:periodogram_full} is an accumulation of all the elements in $\bm{F}$ whereas the denominator is a superposition of complex Gaussian random variables.
